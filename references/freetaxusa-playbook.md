...
> **⚠ UI map version: recorded from 2025-season sessions (imported into TaxWiz Aug 2026).**
> Screen names, field labels, sidebar structure, and error text rot between seasons.
> At the start of every filing session, re-verify this map against the live site before
> trusting it: snapshot the landing page, confirm the section tabs (Personal → Income →
> Deductions/Credits → Misc → Summary → State → Final Steps) and one known screen. Note
> deviations at the bottom of this file with a date.
>
> **Tool names here are generic Playwright-MCP names** (`browser_snapshot`,
> `browser_fill_form`…). Translate per runtime using the adapter table in
> `runtime-notes.md` — in Claude Code prefer claude-in-chrome (the user's real Chrome and
> real login). Never headless, never bot-evasion; assist inside the user's watched session.
>
> **Useful site fact (from the freetaxusa-mcp project):** interview pages are numbered by
> a `?sid=N` query param on `www.freetaxusa.com/taxes<year>/taxcontrol`; the sidebar links
> contain `sid=` and can be enumerated from a snapshot to build a live section map.
