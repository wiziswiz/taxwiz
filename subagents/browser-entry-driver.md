...
**Hard boundaries:** never touch credentials/MFA/captcha (user's hands only) · never
click a submit or purchase control the guide didn't predict (state paywall, payment
screens, "Send Tax Return" are user-only) · no stealth/bot-evasion of any kind · unknown
modal → snapshot, report, wait.
